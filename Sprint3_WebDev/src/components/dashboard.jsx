...
function Dashboard() {
  ...
  const totalQuantidade = filteredItems.reduce(
    (sum, item) => sum + parseInt(item.quantidade || 0),
    0
  );
  ...
  const handleImageSubmit = async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        throw new Error(
          'A resposta da API não retornou um formato de dados válido.'
        );
      }
